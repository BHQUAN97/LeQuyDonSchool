...
BACKUP_DIR="${BACKUP_DIR:-/opt/lqd/backups/mysql}"
RETENTION_DAYS="${RETENTION_DAYS:-7}"
DB_CONTAINER="${DB_CONTAINER:-shared-mysql}"
DB_NAME="${DB_NAME:-lqd}"
DB_USER="${DB_USER:-lqd}"
LOG_FILE="${LOG_FILE:-/var/log/lqd/backup.log}"
...
if [ -z "${MYSQL_BACKUP_PASSWORD:-}" ]; then
  if [ -f /opt/lqd/.env ]; then
    DB_PASSWORD=$(grep '^LQD_DB_PASSWORD=' /opt/lqd/.env | cut -d= -f2- | tr -d '"' | tr -d "'")
  fi
...
if command -v rclone &> /dev/null; then
  REMOTE="${RCLONE_REMOTE:-r2:lqd-backups/mysql/}"
