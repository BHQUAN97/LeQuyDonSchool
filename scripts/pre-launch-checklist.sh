...
SITE_URL="${1:-https://demo.remoteterminal.online}"
...
CATEGORIES=$(curl -s -o /dev/null -w "%{http_code}" "${API_URL}/categories" 2>/dev/null || echo "000")
check "Categories API" "$([ "$CATEGORIES" = "200" ] && echo true || echo false)"

EVENTS=$(curl -s -o /dev/null -w "%{http_code}" "${API_URL}/events?page=1&limit=1" 2>/dev/null || echo "000")
check "Events API" "$([ "$EVENTS" = "200" ] && echo true || echo false)"
...
check "Login endpoint responds (401 or 429)" "$([ "$LOGIN_CODE" = "401" ] || [ "$LOGIN_CODE" = "429" ] && echo true || echo false)"
